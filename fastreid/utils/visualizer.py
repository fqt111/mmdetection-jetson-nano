...
class Visualizer:
    ...
    def get_model_output(self, all_ap, dist, q_pids, g_pids, q_camids, g_camids,q_frameids,g_frameids,q_personids,g_personids):
        self.all_ap = all_ap
        self.dist = dist
        self.sim = 1 - dist
        self.q_pids = q_pids
        ...
        self.q_frameids=q_frameids
        self.g_frameids=g_frameids
        self.q_personids=q_personids
        self.g_personids=g_personids
    ...
    def save_rank_result(self, query_indices, output, max_rank=5, vis_label=False, label_sort='ascending',
        ...
        for cnt, q_idx in enumerate(tqdm.tqdm(query_indices)):
            ...
            frame_id=query_info['frameids']
            person_num=query_info['person_num']
            frame_id_flag=self.add_zeros(frame_id)
            print(frame_id_flag)
            print(person_num)

            query_name = query_info['img_paths'].split('/')[-1]
            flag=False
            with open(rf'./datasets\Market-1501-v15.09.15\\reid\label\c{cam_id+1}s1_{frame_id_flag}.txt', 'r+') as file:
            ...
            for i in range(max_rank):
                ...
                frame_id = gallery_info['frameids']
                person_num = query_info['person_num']
                frame_id_flag = self.add_zeros(frame_id)
                print('print',frame_id,cam_id)
                all_imgs.append(gallery_img)
                gallery_img = np.rollaxis(np.asarray(gallery_img, dtype=np.uint8), 0, 3)
                similarity = self.sim[q_idx, sort_idx[i]]
                if similarity>0.9:
                    label = 'true'
                    ...
                    with open(rf'./datasets/Market-1501-v15.09.15/reid/label/c{cam_id+1}s1_{frame_id_flag}.txt', 'r+') as file:
                        ...
                        msg = line.split(" ")
                        if len(msg) == 4:
                            line = line.replace('\n', '') + f' {person_id if flag else person_count-1}\n'
                        lines[line_num] = line
                        file.seek(0)
                        file.writelines(lines)
